...
int  main(int argc, char* const argv[])
{
	...
	if (argc < 2) {
		printf("слишком мало переданно аргументов программе 'c'\n");
		return 0;
	}
	...
	if ((fd_shm = shm_open(shared_memory_name, O_RDWR , 0660)) == -1) {
		perror("ошибка работы shm_open в программе 'c'\n");
		exit(1);
	}
	...
	if (sem2 == SEM_FAILED || sem3 == SEM_FAILED) {
		perror("ошибка работы sem2 в программе 'c'\n");
		exit(1);
	}
	...
	if (p == 0) {
		...
		if (execve("b.out", argv, NULL) == -1) {
			perror("ошибка выполнения в программе 'c'\n");
			exit(1);
		}
	} else if (p > 0) {
		sem_wait(sem3);
